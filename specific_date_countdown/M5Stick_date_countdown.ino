...
#define TIMEZONE  1 // GMT+1, change it to your time zone

RTC_TimeTypeDef RTC_TimeStruct;
RTC_DateTypeDef RTC_DateStruct;

int year = 2023; //desired year
int month = 3; //desired month
int date = 14; //desired date
...
void setup() {
  ...
  M5.Lcd.printf("Days until set date\n");
}
...
void loop() {
  ...
  tm setDate = {};
  setDate.tm_year = year;
  setDate.tm_mon = month-1; // Month is 0-based
  setDate.tm_mday = date;
  time_t setDateTime = mktime(&setDate);
  ...
  double secondsUntilSetDate = difftime(setDateTime, currentTime);

  // Calculate the number of days until set date
  int daysUntilSetDate = secondsUntilSetDate / 86400;
  

  // display the number of days until set date on the M5StickC screen
  if (prevDay!=daysUntilSetDate){ //update everything when daysUntilSetDate changes
    M5.Lcd.fillScreen(BLACK);
    M5.Lcd.setCursor(20,4);
    M5.Lcd.printf("Days until set date\n");
    ...
    prevDay = daysUntilSetDate;
  }
  if (t!=RTC_TimeStruct.Seconds){ //update only time when seconds change, prevents flickering
